    ...
    <div class="content">
      ...
      <div v-if="this.getSelectedGameRoom.status == 1">
        ...
        <div style="display: flex; flex-direction: row;background-color: var(--accent); align-items: center;justify-content: center; gap:10px;" v-if="this.getPlayerTurn == this.getUserId" class="playerTurn">
          ...
          <span :class="this.getConnectFourPiece" v-if="this.getSelectedGameRoom.gameTypeId===2"></span>
          Twój ruch!</div>
...
export default {
  ...
  computed: {
    ...
    getConnectFourPiece() {
      ...
      if(this.getSelectedGameRoom.players.map(x => x.playerId).indexOf(this.getUserId)===1)
        return 'my-pawn'
      return 'enemy-pawn';
    }
  }
